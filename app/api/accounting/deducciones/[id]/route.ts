...
export async function PATCH(req: NextRequest, ctx: RouteContext) {
  ...
  const ref = db.collection(entity.collections.deducciones).doc(id);
  ...
  try {
    ...
    console.error("[accounting/deducciones PATCH]", e);
...
export async function DELETE(req: NextRequest, ctx: RouteContext) {
  ...
  try {
    await db.collection(entity.collections.deducciones).doc(id).delete();
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("[accounting/deducciones DELETE]", e);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
